...
StatusOr<bool> RematerializeLargeAllGather::Run(
    HloModule* module,
    const absl::flat_hash_set<absl::string_view>& execution_threads) {
  bool changed = false;
  bool tp_replica_group_found = false;
      ...
      if (inst->opcode() == HloOpcode::kGetTupleElement) {
        ...
        if (opt_barrier->opcode() == HloOpcode::kOptimizationBarrier) {
          ...
          if (input_inst->opcode() == HloOpcode::kAllGather &&
            ...
            VLOG(2) << "Will remat all-gather: " << input_inst->ToString();
            // Replace the operand of the optimization barrier's tuple with the
            // AllGather input
            TF_RETURN_IF_ERROR(
                opt_barrier->mutable_operand(0)
                    ->ReplaceOperandWithDifferentShape(
                        tuple_index, input_inst->mutable_operand(0)));
            ...
            *new_tuple_shape.mutable_tuple_shapes(tuple_index) =
                input_inst->mutable_operand(0)->shape();
            ...
            *opt_barrier->mutable_shape() = new_tuple_shape;
            ...
            HloInstruction* new_gte = computation->AddInstruction(
                HloInstruction::CreateGetTupleElement(
                    input_inst->mutable_operand(0)->shape(), opt_barrier,
                    tuple_index));

            HloAllGatherInstruction* all_gather =
                Cast<HloAllGatherInstruction>(input_inst);
