 ...
 public:
  ...
  absl::Status HandleCustomCall(HloInstruction* custom_call) override {
    ...
    if (Match(custom_call, OneDnnMatmulInstr(&matmul))) {
      auto backend_config = matmul->backend_config<BackendConfig>();

      HloInstruction *transpose, *operand;
      auto new_ops = matmul->mutable_operands();

      UpdateTransposeDimensions(matmul, new_ops, 0, &backend_config);
      UpdateTransposeDimensions(matmul, new_ops, 1, &backend_config);

      auto matmul_call = Cast<HloCustomCallInstruction>(matmul->AddInstruction(
          matmul->CloneWithNewOperands(matmul->shape(), new_ops)));
      TF_RETURN_IF_ERROR(matmul_call->set_backend_config(*backend_config));
      TF_RETURN_IF_ERROR(ReplaceInstruction(matmul, matmul_call));
