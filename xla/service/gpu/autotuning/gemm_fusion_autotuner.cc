...
GemmFusionAutotunerImpl::GenerateTritonConfigs(const HloDotInstruction& dot) {
  ...
  for (TritonGemmConfig& config : triton_configs) {
    ...
    // predicates that require additional restriction on block_m to avoid
    // failures similar to b/378660935. It's unclear if the issue extends
    // beyond these cases, so restrictions here are conservative to these.
