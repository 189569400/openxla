...
TEST_F(ExecutionStreamAssignmentTest, CalledComputations) {
  ...
  ENTRY %entry (Arg_0.1.0: f32[2048,2048], Arg_1.2.0: f32[2048,2048]) -> f32[2048,2048] {
    ...
    %call-start = ((f32[2048,2048]{1,0}, f32[2048,2048]{1,0}), f32[2048,2048]{1,0}) call-start(f32[2048,2048]{1,0} %Arg_0.1.0, f32[2048,2048]{1,0} %Arg_1.2.0), to_apply=%stream1.3, frontend_attributes={_xla_stream_annotation="2"}, metadata={scheduling_name="call-start"}, backend_config={"operation_queue_id":"1","wait_on_operation_queues":[],"force_earliest_schedule":false}
    %call-done = f32[2048,2048]{1,0} call-done(((f32[2048,2048]{1,0}, f32[2048,2048]{1,0}), f32[2048,2048]{1,0}) %call-start), metadata={scheduling_name="call-done"}, backend_config={"operation_queue_id":"1","wait_on_operation_queues":[],"force_earliest_schedule":false}
    %call-start.2 = ((f32[2048,2048]{1,0}, f32[2048,2048]{1,0}), f32[2048,2048]{1,0}) call-start(f32[2048,2048]{1,0} %Arg_0.1.0, f32[2048,2048]{1,0} %call-done), to_apply=%stream1.4, frontend_attributes={_xla_stream_annotation="1"}, metadata={scheduling_name="call-start"}, backend_config={"operation_queue_id":"1","wait_on_operation_queues":[],"force_earliest_schedule":false}
    ROOT %call-done-2 = f32[2048,2048]{1,0} call-done(((f32[2048,2048]{1,0}, f32[2048,2048]{1,0}), f32[2048,2048]{1,0}) %call-start.2), metadata={scheduling_name="call-done"}, backend_config={"operation_queue_id":"1","wait_on_operation_queues":[],"force_earliest_schedule":false}
}
  )";
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
                          ParseAndReturnVerifiedModule(kModuleStr));
  ...
  // Same checks as above but now on stream #1.
  ExpectExecutionStreamForSyncInstructions(
      assignment,
